...
export default function EmailPortalPage() {
  ...
  const batches = data.batches.filter((b) => b.channel === "email")
  const templates = data.templates.filter((tp) => tp.channel === "email")
  ...
  const audience =
    segment === "all"
      ? data.customers.filter((c) => c.consent.email).length
      : data.customers.filter(
          (c) => c.consent.email && c.segment.en === segment
        ).length
  const cost = audience * 0.05
  ...
  const kpis: KpiCell[] = [
    ...
    },
    {
      ...
      value: data.customers.filter((c) => c.consent.email).length,
      color: "var(--chart-3)",
    },
  ]
  ...
  return (
    <ScrollFade className="min-h-0 flex-1">
      <PageHeader
        title={t("messaging.emailTitle")}
        subtitle={t("messaging.emailSubtitle")}
      />
